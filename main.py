...
def f_Fck():
    ...
    for i in range(a):
        while True:
            element = float(input(f'Введите значение коэффицента для {i + 1} груза: '))
            if element > 0:
                break
...
def f_Fnc():
    for i in range(a):
        while True:
            element = float(input(f'Введите среднесуточное поступление или отпуск материала для груза {i + 1 } в (в т/сут): '))
            if element > 0:
                break
